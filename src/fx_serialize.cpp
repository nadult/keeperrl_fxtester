...
namespace {
	...
	template <class T, int vec_size = T::vec_size>
	vector<T> delinearize(const vector<float> &values) {
		vector<T> out;
...
void ParticleDef::serialize(IArchive &ar, unsigned int) {
	ar(CEREAL_NVP(life), CEREAL_NVP(alpha), CEREAL_NVP(size), CEREAL_NVP(slowdown),
	   CEREAL_NVP(color));
}

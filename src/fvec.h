...
template <class T> auto dot(const vec2<T> &lhs, const vec2<T> &rhs) {
...
template <class T> auto distance(const vec2<T> &lhs, const vec2<T> &rhs) {
	return length(lhs - rhs);
}

template <class T> auto distanceSq(const vec2<T> &lhs, const vec2<T> &rhs) {
...
template <class T> vec3<T> vinv(const vec3<T> &vec) {
	return {T(1) / vec[0], T(1) / vec[1], T(1) / vec[2]};
}
...
template <class T> auto cross(const vec2<T> &a, const vec2<T> &b) {
